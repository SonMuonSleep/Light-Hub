...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì Light X Hub đã TỚI!!!!!<Color=/>"):Display()
    wait(.14)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Channel : GiaTocGAMING <Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "Light X Hub", HidePremium = false, IntroText = "NightHub Library", SaveConfig = true, ConfigFolder = "Light X Hub"})
...
OrionLib:MakeNotification({
	Name = "Light X Hub",
	Content = "Light X Hub Not Skidder, Please don't says",
	Image = "rbxassetid://4483345998",
	Time = 25
})
...
W:AddLabel("Script By GiaTocGAMING")

local Section = W:AddSection({
    Name = "Update"
})

W:AddLabel("[+] Light X Hub is COME!")
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Light X Hub(Noitification)",
